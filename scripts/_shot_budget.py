...
def recommended_shot_range(duration: int) -> tuple[int, int]:
    """Return the fast-drama subshot target for one generated clip.

    The current production baseline is the user's 10,963-shot corpus: mean
    cut length is about 2.1 seconds (2s ~= 55%, 3s ~= 30%).  Shot count is a
    rhythm target, not a reason to delete story beats.
    """
    if duration <= 0:
        return (1, 8)
    if duration <= 4:
